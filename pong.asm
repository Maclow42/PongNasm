...
move_dir db DIR_NONE
future_move_dir db DIR_NONE
...
handle_key:
	...
	.none:
		mov byte [future_move_dir], DIR_NONE
		jmp .exit
...
move_snake:
	...
	cmp al, DIR_UP	; if current mov is UP
	je .up

	cmp al, DIR_DOWN ; if current mov is DOWN
	je .down
...
update_map_snake:
	mov rcx, [length]
	sub rcx, 1			; rcx = length-1

	mov byte [snake_cells_buf+rcx*8], MAP_FREE

	neg rcx				; rcx = -(length-1) = -length+1

	mov byte [snake_cells_buf+rcx*8], MAP_FREE
	
	; get current new head position
	mov rax, [snake_x]
	mov rdx, [snake_y]
